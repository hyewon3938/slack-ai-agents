...
- **개인 프로젝트에 팀 수준 품질 관리** — 295개 테스트(인사이트 엔진 TDD), GitHub Actions CI/CD, Public 저장소 보안 방어.
...
| **DB 접근** | — | 직접 연결 | HTTPS API 프록시 (DB 포트 미노출) |
| **배포** | — | docker compose (8\~9분) | yarn deploy (2분) + GitHub push |

- **v1→v2**: Notion은 테이블 간 JOIN이 안 돼서 크로스 분석이 불가능. 코어(데이터층 + 모델 + 에이전트 구조)를 통째로 교체.
- **v2→v3**: 웹 대시보드 추가로 Docker 서비스가 4개로 팽창. 웹을 Vercel로 분리하고, DB는 VM에 PostgreSQL Docker로 유지. Vercel → VM DB 직접 연결을 HTTPS API 프록시로 전환하여 DB 포트 외부 노출을 완전 제거.
...
| Proxy      | Caddy (자동 TLS) + Node.js DB Proxy API                         |
| CI/CD      | GitHub Actions + Vercel + yarn deploy                            |
| Security   | DB 프록시 + Rate Limiting + SQL 감사 로그 + TLS + 프롬프트 인젝션 차단 |
| Test       | vitest (295개 테스트)                                            |
...
```
Vercel (웹) ──HTTPS──→ Caddy (자동 TLS) ──→ DB Proxy API (:3100) ──→ PostgreSQL (내부)
                                                                         ↑
Slack Bot (같은 VM) ─────────────────── localhost ──────────────────────────┘
```

- PostgreSQL은 Docker `expose` 로 내부 네트워크에만 개방 (외부 포트 바인딩 없음)
- Vercel → DB 직접 연결을 완전히 제거하고, **HTTPS API 프록시**를 경유
- Caddy가 Let's Encrypt 인증서를 자동 발급·갱신하여 전 구간 TLS 암호화
- DB Proxy는 Bearer API Key 인증 + 1MB 요청 크기 제한 + 헬스체크 엔드포인트
...
| 방어 계층 | 구현 |
|----------|------|
| **user_id 동적 격리** | Slack 사용자 → DB users 테이블 매핑 — 모든 쿼리에 요청자의 userId 필터 강제, 다른 값 사용 시 즉시 차단 |
| **SQL 감사 로그** | 모든 도구 호출을 기록 (도구명, 트렁케이트된 SQL, 결과/에러) |
| **DDL 차단** | DROP, TRUNCATE, ALTER, CREATE, GRANT, REVOKE 원천 차단 |
| **벌크 방어** | UPDATE/DELETE에 WHERE 필수 + 최대 50행 제한 |
...
| 04-09     | **다중 사용자 격리** — Slack→DB userId 동적 매핑, 모든 쿼리 userId 파라미터화, DB 프록시 DDL 차단·CORS 제한 |
...
└── shared/
    ...
    ├── user-resolver.ts          # Slack 사용자 → DB userId 매핑
    ├── kst.ts                    # KST 타임존 유틸리티
