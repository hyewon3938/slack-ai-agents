...
> "월말마다 컨디션이 떨어지는 것 같아"가 진짜 내 패턴인지 그냥 그렇게 기억하는 건지, 혼자 살면 객관적으로 짚어줄 시선이 없다. 그 답은 내 감이 아니라 매일 쌓인 기록에서 통계로 나온다.
>
> 게다가 무엇이 맞는지는 사람마다 다르다. 누구는 7시간을 자도 피곤하고 누구는 6시간이면 충분하니, 남들 평균이 아니라 내 데이터(n=1)에 맞춘 기준이라야 한다.
>
> 일정·루틴·수면·일기를 자연어로 편하게 입력하면 에이전트가 그날그날 일상을 챙기고, 기록이 쌓이면 내가 몰랐던 패턴을 통계로 검증해 먼저 짚어준다. 사용자는 편하게 기록·관리하려고 쓸 뿐이지만, 그 기록에서 패턴을 찾아내는 게 이 서비스의 핵심이 되도록 흐름을 설계했다. 2026-03-05 시작, 매일 사용 중.
...
무엇이 맞는지도 사람마다 다르다. "하루 8시간을 자라" 같은 일반론은 누구에겐 맞고 누구에겐 안 맞으니, 남들 평균이 아니라 내게서 나온 데이터(n=1)로 세운 내 기준으로 본다.
...
### 발굴과 승인 — 추적할지는 사람이 고른다

발굴 엔진이 아직 아무도 안 엮은 (시드 × 신호) 여집합을 훑어, 느슨한 발견 기준을 통과한 후보를 슬랙 승인 카드로 띄운다. [추적 시작]이면 다음 주부터 검증을 누적하고 [패스]면 묻힌다. 한 묶음을 전부 패스하면 다음 날 다음 best 후보가 다시 올라온다.
...
> 아직 **후보**일 뿐 — 카드에도 "연관이지 인과 아님"이라 적힌다. 추적에 들어간 가설이 아래 검증을 통과해야 '검증됨'이 된다.

### 검증 결과 — 측정이 정확해지면 등급도 내려간다

검증 구조가 실제로 도는지는 "맞는 걸 얼마나 잡았나"보다 **틀릴 수 있을 때 스스로 내리는가**로 드러난다. 최근 검정 구간을 정밀화하면서 — 실데이터가 존재하는 기간으로 자르고, 발굴로 찾은 가설은 등록된 다음부터만 채점하도록(선택 편향 차단) — 전체 패턴을 한 번 재기준선했다. 그러자 그전까지 '검증됨'으로 보던 패턴이 실데이터 구간이 짧아 풀 게이트를 못 넘는다는 게 드러나 **검증 중으로 다시 내려갔다**. 기준을 높인 게 아니라 측정이 정확해진 결과다.

그래서 지금 풀 게이트(자기상관·다중검정 보정 + 효과크기 + 누적 e-value 20을 **동시에** 통과)를 넘은 패턴은 **0개**, e-value 20을 넘은 것도 **0개**다 — 기록 3개월차에 일부 신호(예: 일기 메타 태그)는 실데이터가 아직 며칠\~몇 주뿐이라 통계로 확정하긴 이르다. 과대 단언 대신 "아직"이라고 말하는 게 이 프로젝트의 기준이다.

대신 침묵하지는 않는다. 가능성이 보이는 패턴은 **검증 중**(emerging)으로 매일 계속 노출하고, 확정까지 얼마나 왔는지 e-value 진행바로 보여준다. 데이터가 쌓이면 같은 기준으로 다시 올라온다. (구체 패턴·수치·사주 원국은 개인 데이터라 비공개.)
