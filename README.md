...
| Day | 날짜        | 내용                                                                        |
| --- | ----------- | --------------------------------------------------------------------------- |
| 1   | 03-05       | TypeScript + ESM, Slack Bolt, LLM 추상화, MCP 클라이언트                    |
| 2   | 03-06       | 일정 에이전트, 크론 알림, 루틴 에이전트, Docker 배포                        |
| 3   | 03-07       | SDK 직접 조회(7\~11초→\~1초), 의도 분류 진화, 대화 히스토리                   |
| 4   | 03-08       | **v2 전환** — PostgreSQL, KST 수정, App Home, 스마트 메모리                 |
| 5   | 03-09       | Hooks/Skills, 비용 최적화, 생활 맥락 잔소리, 개발 크론                      |
| 6   | 03-10\~11   | Next.js 캘린더, DnD, 백로그, 카테고리, PWA, 다층 보안 체계, HTTPS 배포      |
| 7   | 03-12       | UX 개선, **v3 전환**(Vercel+Neon, Docker 4→1), CI/CD 배포 알림, 캐싱, 테스트 |
