...
- **웹 대시보드**: Next.js 15 기반 캘린더 UI로 Slack 에이전트가 쌓은 데이터를 시각화하고 직접 조작한다. 동일 PostgreSQL을 공유하며, Caddy 리버스 프록시를 통해 HTTPS로 배포한다.
...
![수면 기록 대화 — 주간 패턴 리뷰](docs/images/04-sleeping.jpg)
...
Slack 대화만으로 부족했던 **시각적 일정 관리**를 위해 Next.js 15 기반 웹 대시보드를 구축했다. Slack 에이전트가 쌓은 데이터를 캘린더·백로그·카테고리 뷰로 시각화하고, 드래그 앤 드롭으로 직접 조작할 수 있다. HTTPS 도메인으로 배포하여 모바일에서도 접근 가능하고, PWA를 지원해 홈 화면에 추가할 수 있다.
...
![App Home 대시보드 — 수면 요약 + 루틴 현황](docs/images/03-app-home.jpg)
...
| DevOps     | Docker + Caddy (HTTPS) + Oracle Cloud Free Tier ARM VM |
| CI/CD      | GitHub Actions (PR 체크 + main 머지 시 자동 배포)      |
...
### 개발 크론 — AI 자율 모니터링 (node-cron, 2개)

| 작업        | 스케줄     | 내용                                                                |
| ----------- | ---------- | ------------------------------------------------------------------- |
| devReview   | 매일 09:00 | GitHub API 커밋 수집 → Claude Opus 패턴 분석 → DB 저장 → Slack 전송 |
| workSummary | 매일 09:05 | GitHub API 커밋 수집 → Gemini Flash 팩트 요약 → Slack 전송          |
...
8일간의 집중 개발로 v1 설계 → 운영 → 한계 인식 → v2 전환 → 웹 대시보드 → HTTPS 배포까지 진행했다.
...
| Day 5 (03-09)    | AI 워크플로우 + 고도화 | Hooks/Skills, 비용 최적화, 생활 맥락 잔소리, 개발 크론     |
...
│   └── dev-cron.ts               # 개발 크론 (devReview + workSummary)
└── shared/
