...
기록이 석 달쯤 쌓일 때까지는 대부분 아직 증거가 충분히 모이지 않아 패턴으로 확정되지 않는다. 통계로 잡힐 만큼 강한 연관이 별로 없으면, 억지로 단언하는 대신 아직 모른다고 정직하게 말한다. 그렇다고 아무것도 안 보여주는 건 아니다. 확정 전이라도 가능성이 보이면 **검증중**(emerging)으로 계속 보여주고, 확정까지 얼마나 왔는지 e-value 진행바(`e=4.2/20`)로 표시한다.

매일 아침, 그날 켜진 시드를 **얼마나 증거가 쌓였는지**에 따라 세 단계로 나눠 보여준다. 확정된 패턴이면 단언하고, 쌓는 중이면 조심스럽게 말하고, 아직 근거가 없으면 주장 없이 켜졌다고만 알린다. 확신이 어느 정도인지를 말투로 구분해서, 검증 안 된 걸 확정처럼 말하지 않으려는 것이다.
...
> **시드는 '관찰할 조건'이고, 두 곳에서 가져온다.** 하나는 **생활 통념**으로, 요일·주말·월말·계절·수면 부족처럼 누구나 영향을 의심해볼 만한 환경 변수를 빠짐없이 후보로 깔아둔다. 다른 하나는 **사주**로, 태어난 사주에서 정해지는 글자가 그날 운에 들어오는 날이다. 평소 관심이 가던 틀이라, 막연히 믿거나 무시하는 대신 검증대에 올렸다. 어느 쪽이든 똑같은 off-day 통계로 채점되니 사주라고 특별할 건 없다.
