...
- [기술 스택](#기술-스택)
- [설계 노트](#설계-노트)
- [보안 아키텍처](#보안-아키텍처--public-저장소--개인-데이터)
...
- **프로덕션 보안 설계** — Public 저장소 + 개인 데이터 환경에서 DB 포트 외부 미노출, HTTPS API 프록시, SQL 감사 로그, Rate Limiting, 프롬프트 인젝션 차단까지 다층 방어 구축.
- **1인 풀스택** — Slack 에이전트 + Next.js 대시보드(DnD, PWA) + Docker/VM + Vercel + PostgreSQL + Caddy(TLS).
- **개인 프로젝트에 팀 수준 품질 관리** — 320개 테스트(인사이트 엔진 TDD), GitHub Actions CI/CD(GHCR 이미지 빌드·자동 정리), Public 저장소 보안 방어.
...
Vercel(웹)은 DB에 직접 연결하지 않고, **HTTPS API 프록시**를 경유해 데이터를 조회한다. DB 포트는 외부에 노출되지 않으며, 모든 통신은 TLS로 암호화된다.
...
| **인프라** | Docker 1개 | Docker 4개 (app+db+web+caddy) | Docker 2개(app+db) + 호스트 Caddy + Vercel(웹) |
| **DB 접근** | — | 직접 연결 | HTTPS API 프록시 (DB·API 포트 외부 미노출) |
| **배포** | — | docker compose (8\~9분) | GitHub Actions → GHCR 이미지 빌드·자동 정리 → VM pull + 재기동 |

- **v1→v2**: Notion은 테이블 간 JOIN이 안 돼서 크로스 분석이 불가능. 코어(데이터층 + 모델 + 에이전트 구조)를 통째로 교체.
- **v2→v3**: 웹 대시보드 추가로 Docker 서비스가 4개로 팽창. 웹을 Vercel로 분리하고, Caddy는 컨테이너에서 빼내 호스트 systemd 서비스로 이동(컨테이너 재배포 시 TLS 인증서 영향 분리). DB·API 포트는 호스트 루프백에만 바인딩하고, 외부 트래픽은 반드시 Caddy를 거쳐 HTTPS로만 도달하도록 구조화.
...
| Bot Infra  | Docker Compose (app + db) + 클라우드 리눅스 VM                    |
| Web Infra  | Vercel (자동 배포, HTTPS 내장)                                   |
| Proxy      | Caddy (호스트 서비스, Let's Encrypt 자동 TLS) + Node.js DB Proxy API |
| CI/CD      | GitHub Actions (GHCR 이미지 빌드·푸시·자동 정리) + Vercel          |
| Security   | DB 프록시 + 루프백 포트 바인딩 + Rate Limiting + SQL 감사 로그 + TLS + 프롬프트 인젝션 차단 |
| Test       | vitest (320개 테스트)                                            |
...
## 보안 아키텍처 — Public 저장소 + 개인 데이터

> 코드가 전부 공개된 상태에서 개인 일정·수면·루틴 데이터를 안전하게 지키기 위한 다층 방어 설계.

이 프로젝트는 Public 저장소이면서 실제 개인 데이터를 다루기 때문에, **"코드가 보여도 안전한" 아키텍처**를 설계해야 했다. 코드 구조, 배포 설정, API 엔드포인트가 모두 공개되어 있으므로 단순한 정보 은닉이 아닌 구조적 방어가 필요했다.

### 네트워크 — 외부 노출면 최소화

```
Vercel (웹) ──HTTPS──→ Caddy (호스트 서비스, 자동 TLS) ──→ DB Proxy API (루프백:3100) ──→ PostgreSQL (Docker 내부)
                                                                                            ↑
Slack Bot (같은 Docker 네트워크) ────────────────────────── 내부 통신 ────────────────────────┘
```

- PostgreSQL은 Docker `expose`로 내부 네트워크에만 개방 (호스트 포트 바인딩 없음)
- DB Proxy API 포트(3100)는 `127.0.0.1:3100`에만 바인딩 → VM 공개 IP에서 직접 도달 불가. 외부 트래픽은 반드시 호스트 Caddy의 TLS 종료를 거쳐야 함
- Vercel → DB 직접 연결을 완전히 제거하고, **HTTPS API 프록시**를 경유
- Caddy가 Let's Encrypt 인증서를 자동 발급·갱신하여 전 구간 TLS 암호화. 컨테이너 재배포와 인증서 갱신 라이프사이클 분리
- DB Proxy는 Bearer API Key 인증(타이밍 세이프 비교) + 1MB 요청 크기 제한 + SQL 화이트리스트 검증 + 헬스체크 엔드포인트

### 애플리케이션 — LLM SQL 실행 방어

LLM이 SQL을 직접 작성하는 구조에서 발생할 수 있는 위협을 다단계로 차단:

| 방어 계층 | 구현 |
|----------|------|
| **테넌트 격리 검증** | LLM이 생성한 SQL에 소유자 필터 삽입 여부를 실행 전 검증 — 누락 또는 위조 시 즉시 거부. 대시보드는 별도로 iron-session 기반 세션 userId로 격리 |
| **SQL 감사 로그** | 모든 도구 호출을 기록 (도구명, 트렁케이트된 SQL, 결과/에러) |
| **DDL 차단** | DROP, TRUNCATE, ALTER, CREATE, GRANT, REVOKE 원천 차단 |
| **위험 함수 차단** | `COPY`, `pg_read_file`, `dblink`, `pg_sleep` 등 파일 I/O·외부 연결·타이밍 공격 벡터 차단 |
| **벌크 방어** | UPDATE/DELETE에 WHERE 필수 + 최대 50행 제한 (트랜잭션 래핑) |
| **타임아웃** | 파라미터화된 `statement_timeout` (10초) — SQL Injection 벡터 제거 |
| **프롬프트 인젝션** | `custom_instructions` 삽입 시 "ignore previous" 등 공격 패턴 감지·차단 |

### 요청 제어 — Rate Limiting

- **슬라이딩 윈도우 Rate Limiter**: 사용자당 5req/min (인메모리, 10분 주기 가비지 컬렉션)
- **메시지 크기 제한**: 10KB 초과 메시지 차단
- **봇 메시지 필터링**: 무한 루프 방지

### 개발 프로세스 보안

- **커밋 전 시크릿 스캔** (Hook): API 키, 토큰, 비밀번호 패턴 자동 감지
- **코드 리뷰 보안 감사** (Skill): PR 생성 시 보안 체크리스트 자동 수행
- **PostgreSQL TLS**: Docker 컨테이너 간 통신도 SSL 인증서로 암호화
...
코드 레벨 방어(위 [보안 아키텍처](#보안-아키텍처--public-저장소--개인-데이터) 참조) + 개발 프로세스 방어: CLAUDE.md(최상위 규칙) → conventions.md(체크리스트) → Hook(커밋 전 스캔) → Skill(리뷰 시 감사). 보안 이슈는 즉시 수정.
...
| 04-10     | **배포 파이프라인 최적화** — GitHub Actions → GHCR 이미지 빌드 → VM pull + 재기동. Warm cache 총 파이프라인 \~81초로 고정, 의존성 변경 시 최악 케이스(4\~10분 + 타임아웃 실패) 제거, 편차 13배 이상 → 2배 내로 축소. 후속: GHCR 이미지 자동 정리(최근 10개 유지), VM 로컬 이미지 보존 정책(최신 2개) |
| 04-10     | **보안 아키텍처 점검** — DB Proxy API 포트(3100) 호스트 루프백 바인딩으로 전환(Caddy 경유 강제). README 인프라 클레임 정합성 점검 및 보정 |
...
└── shared/
    ...
    ├── db.ts                     # PostgreSQL 연결 + 쿼리 (TLS)
    ├── sql-tools.ts              # SQL 도구 (감사 로그 + user_id 검증 + DDL 차단)
    ├── rate-limiter.ts           # 슬라이딩 윈도우 Rate Limiter
...
│   ├── login/                    # 인증
│   └── api/                      # API Routes
└── src/lib/db.ts                 # DB Proxy 클라이언트 (HTTPS fetch)
```
