# 매일의 기록에서 "진짜 내 패턴"을 통계로 검증하는 라이프 로그 시스템

> "월말마다 컨디션이 떨어지는 것 같아"가 진짜 내 패턴인지, 그냥 그렇게 기억하는 건지. 매일 쌓은 기록으로 통계가 대신 가려준다.
>
> 일정·루틴·수면·일기·지출을 자연어로 편하게 기록하면 Claude가 그날그날 생활을 챙겨주고, 기록이 쌓일수록 시드 켜진 날 vs 안 켜진 날을 대조(off-day)하고 e-value로 확정한 내 생활 패턴이 덤으로 드러난다.
>
> 사용자는 편하게 기록만 하면 된다. 무엇이 진짜 패턴인지 발굴·검증·노출할지는 통계가 정한다. 2026-03-05 시작, 매일 사용 중.
...
<p align="center">
  ...
  <img src="https://img.shields.io/badge/LLM_생성·통계_판정-2d3748?style=flat-square" alt="LLM 생성·통계 판정" height="40" />
...
혼자 지내는 시간이 길어지면 나를 객관적으로 봐줄 시선이 줄어든다. 곁에 누가 있으면 "요즘 너무 늦게 자더라" 한마디로 방향이라도 잡는데, 그게 없으면 내 상태를 내 기준으로만 보게 된다. 문제는 그 기준마저 편향돼 있다는 거다. 잘 기억나는 것만 떠오르고, 믿고 싶은 대로 해석한다.

그래서 그 객관적 시선을 데이터로 대신하기로 했다. 매일의 기록을 통계로 따져서, 막연히 "이런 것 같다"고 여기던 걸 진짜인지 기분 탓인지 가려준다. 몰랐던 패턴이 잡히면 인정하거나 고쳐볼 거리가 생기고, 근거가 약하면 약하다고 정직하게 알려준다.

그래서 이 프로젝트는 두 얼굴을 가진다.

- **사용자에겐** — 매일의 기록을 편하게 돕고 그날그날 생활을 챙겨주는 도구.
- **시스템 안에선** — 그 기록 위에서 검증된 내 패턴을 찾아내는 통계 엔진.

이 둘은 충돌하지 않는다. 사람들은 통계 엔진을 먹이려고 매일 기록하지 않는다. 기록이 그 자체로 쓸모 있으니까 매일 하는 거고, 그렇게 쌓인 **진짜 기록**이 n=1 패턴 검증을 가능하게 한다. 제품의 쓸모가 엔진의 신뢰도를 만든다.
...
- **off-day 대조** — 시드(관찰할 조건)가 켜진 날만 보면 "원래 자주 있는 일"을 내 패턴으로 착각한다. 그래서 시드 **켜진 날 vs 안 켜진 날**(off-day)을 같은 잣대로 대조한다. 이 baseline이 "기분 탓"과 "진짜 패턴"을 가른다.
- **e-value 확정 게이트** — 매주 리포트를 들여다보면 우연히 유의해 보이는 주가 언젠가 나온다(peeking). 고정 임계로 "이번 주 유의하니 확정"하면 그 우연이 새어든다. 누적 e-value가 **20**을 넘은 것만 "검증됨"이다(= 유의수준 0.05의 역수, 언제 멈춰 봐도 거짓양성이 통제됨).
- **생성·판정·큐레이션 분리** — LLM은 가설·신호를 **생성**만, 채택·기각은 **통계**가 판정, 노출할지는 **사람**이 큐레이션. "LLM이 만든 게 진짜 맞나"를 사람이 매번 검증하지 않아도 되도록 신뢰 비용을 시스템에 외주화한다.
...
### n=1을 다루는 통계 — 자기기만 하나씩에 대한 방어

나 한 사람의 데이터(n=1)로 약한 신호를 잡으면 착각하기 쉽다. 통계 스택의 각 조각은 **착각 하나씩을 막기 위한 장치**다.

| 통계 장치 | 막는 자기기만 |
|---|---|
| off-day 대조 | "원래 자주 있는 일"을 내 패턴으로 착각 (켜진 날만 보면 속는다) |
| 누적 e-value | 매주 들여다보다 "우연히 좋아 보인 주"에 속는 것 (peeking) |
| block permutation | 며칠 이어지는 흐름이 만드는 가짜 신호 (자기상관) |
| empirical-Bayes 수축 | 데이터 몇 개 안 되는데 섣불리 확신 (소표본 과신) |
| BH-FDR | 신호 여럿을 보다 하나가 순전히 운으로 유의해 보이는 것 (다중 검정) |
| Mann-Whitney | 숫자를 통과/실패로만 나누다 버린 정보 되살리기 |

### 느린 수율 = 정직

데이터가 \~90일쯤이면 대부분 "증거 불충분"이고 확정은 거의 없다. 이건 실패가 아니라 *정답*이다. 검출할 만큼 강한 연관이 별로 없으면 빈손을 내미는 대신 "아직 모른다"를 정직하게 내놓는다. 그렇다고 침묵하지는 않는다. 확정 전이어도 가능성이 보이면 **검증중(emerging)** 단계로 계속 노출하고, 확정까지 얼마나 왔는지 e-value 진행바(`e=4.2/20`)로 보여준다.

엄격한 게이트는 "검증됨"이라는 *단언*에만 걸고, 가시성은 hedged한 "검증중"이 따로 나른다. 그래서 몇 달씩 침묵하지도, 미검증을 확정처럼 말하지도 않는다.
...
패턴은 꾸준한 기록에서만 나온다. 그래서 기록 자체가 마찰 없이 편하고, 그 자체로 매일 쓸모 있어야 한다. 사용자 입장에서 이건 "패턴 분석 도구"가 아니라 **생활을 챙겨주는 기록 도구**다. 패턴은 그 위에 덤으로 쌓인다.
...
**그날그날 챙겨준다 (하루 2회 잔소리).** 아침엔 어제 루틴 달성도 + 오늘 일정, 밤엔 그날 데이터를 엮은 잔소리. 검증된 패턴은 여기 tier별로 실려서, 쓸수록 잔소리가 똑똑해진다.
