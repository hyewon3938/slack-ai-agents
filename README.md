...
앞의 검증 구조가 실제로 결과를 내고 있다. 운영하며 쌓인 n=1 데이터에서, 자기상관과 다중검정을 보정한 뒤에도 살아남는 걸 기준으로 보수적으로 거르는데 — 그 게이트를 넘어 효과크기가 뚜렷한(e-value 20↑) 패턴이 **사주 신호 4개 · 생활 신호 1개** 잡혀 있다. 예를 들어 *특정 십성 기운이 강한 날 배달 지출이 늘거나*, *수면이 부족한 날 건강 불편을 더 호소하거나*, *사주 강도가 약한 날 저에너지·낮잠이 몰리는* 식이다. (구체 수치·사주 원국은 개인 데이터라 비공개.)
