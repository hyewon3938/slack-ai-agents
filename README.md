...
- **개인 프로젝트에 팀 수준 품질 관리** — 245개 테스트(인사이트 엔진 TDD), GitHub Actions CI/CD, Public 저장소 보안 방어.
...
| **배포** | — | docker compose (8\~9분) | GitHub Actions → GHCR 이미지 빌드 → VM pull + 재기동 |
...
| CI/CD      | GitHub Actions (GHCR 이미지 빌드/푸시) + Vercel                   |
...
| 방어 계층 | 구현 |
|----------|------|
| **user_id 격리** | 모든 쿼리에 `user_id = 1` 필터 강제 — 다른 값 사용 시 즉시 차단 |
