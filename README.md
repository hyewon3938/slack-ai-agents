...
> 4년차 프론트 개발자가 AI를 활용해 풀스택 영역으로 확장한 실전 기록.

- **Dogfooding 기반 프로덕트 사고** — 내가 매일 쓰는 실제 도구. "쓰다 보니 불편한 지점"이 곧 이슈가 되고 다음 PR이 된다. 5주 연속 일일 사용.
- **Production-grade LLM 엔지니어링** — LLM이 SQL을 직접 쓰는 에이전트 구조에서 발생하는 비용·정확도·안전 문제를 운영하며 하나씩 해결.
