...
> "일찍 자야 일정 다 해내" 같은 잔소리를 먼저 건넨다. 기획·보안·운영까지 1인, 5주째 매일 사용.

![자연어 대화로 일정 등록 + 잔소리](docs/images/01-conversation.png)

<p align="center">
  <b>5주 매일 사용</b> · <b>A to Z 1인 제작</b> · <b>AI 실수 잡는 하네스</b> · <b>Public × 민감데이터 보안</b> · <b>설계 판단 기록(ADR)</b>
</p>
...
- **프로액티브 인사이트 + 생활 맥락 잔소리** — 직접 설계한 5가지 SQL 패턴(연속 기록·수면 추세·빈 시간대 등)을 크론이 자동 감지해, 묻지 않아도 먼저 건넨다. 일기 × 사주 × 수면 크로스 분석도 주간 자동 실행.
- **LLM × SQL 자율 에이전트 + 하네스** — Claude가 SQL 도구로 DB를 직접 다루는 구조에서 발생하는 할루시네이션·비용·안전 문제를 검증·프록시·승인 플로우로 제어.
- **Public 저장소 × 민감 데이터 다층 보안** — 코드가 전부 공개된 상태에서 개인 일정·수면·루틴·일기 데이터를 지키는 6-layer 구조적 방어.
...
- **수면 × 루틴 × 일정 상관 분석** — 수면 부족한 날 루틴 달성률·일정 소화율이 떨어지는 패턴을 추적. 잔소리의 근거가 된다.
- **개인 프로파일 기반 피드백 루프** — 반복되는 체감·테마를 `life_themes`에 누적, 장기 관찰된 상관 패턴을 `saju_patterns`에 활성화/비활성화. 활성 패턴은 이후 응답 프롬프트에 자동 주입되어 개인화된 해석을 제공 (자가 보정 구조).
...
### 2. LLM × SQL 자율 에이전트 + AI 실수 잡는 하네스
...
- **피드백 루프 구조** — 일기에서 반복 감지되는 테마/체감이 `life_themes` · `saju_patterns`에 누적되고, `detection_count` · `confidence` 기반으로 자가 강화/약화. 활성 패턴은 이후 응답 프롬프트에 실시간 주입되어 개인화된 해석 제공.
