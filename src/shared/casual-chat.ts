...
${buildChatPrompt(role)}${historyBlock}`,
      },
...
export const classifyMessage = async (
  ...
  return classifyIntent(llmClient, text, agentContext, role, historyContext);
};
...
export const respondCasualChat = async (
  ...
  const historyBlock = historyContext ? `\n${historyContext}\n` : '';
  const messages: LLMMessage[] = [
    { role: 'system', content: `${buildChatPrompt(role)}${historyBlock}` },
    { role: 'user', content: text },
  ];
