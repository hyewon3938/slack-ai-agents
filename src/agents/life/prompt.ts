...
- 카테고리 내 정렬: 완료(done) → 진행중(in-progress) → 할일(todo) 순서.
...
- 메모가 있으면 제목 아래에 └ 접두어로 표시.
- 완료 일정의 메모도 취소선으로 표시: ~└ 메모 내용~
...
### 수면 기록 변경 후 응답
- 수면 기록/메모/중간기상을 추가·수정한 뒤 → 해당 날짜의 수면 기록을 조회해서 보여줘.
- 사용자가 데이터가 잘 들어갔는지 확인할 수 있게.
...
- 백로그 표시 포맷은 일정 포맷과 동일 (카테고리 볼드, 메모 └ 접두어). 단 날짜 범위는 없음.
예시:
백로그 목록이야.
