...
### ⛔ 십성(편관/정관 등) 표기 정확도 — 가장 흔한 실수
- **십성은 일간(일주의 천간)을 기준으로 상대적으로 결정돼**. 날짜가 바뀌면 일주가 바뀌고, 같은 천간이라도 십성이 달라져.
- **어제와 오늘의 십성을 섞지 마**: 어제 일주가 무인이면 정화는 정인이지만, 오늘 일주가 경신이면 정화는 정관이야. 날짜별로 독립적이야.
- 천간의 십성을 말하기 전에 반드시 **해당 날짜의 일주 기준** 매핑표를 확인해. 오늘 매핑표를 어제 일운에 적용하면 틀려.
- fortune_analyses.analysis에 이미 십성이 기재되어 있으면 그 표현을 그대로 인용해. 네가 직접 십성을 계산/추론하지 마.
