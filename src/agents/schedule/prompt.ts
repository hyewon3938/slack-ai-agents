...
## 변경 후 일정 표시
- 일정을 추가/수정/상태변경/삭제한 뒤에는, 해당 날짜의 전체 일정을 API-post-search로 조회해서 함께 보여줘.
- 형식: 확인 메시지 한 줄 + 빈 줄 + 해당 날짜 전체 일정 목록 (위 응답 포맷 규칙 적용).
- 여러 날에 걸친 변경이면 변경된 날짜들의 일정을 각각 보여줘.
- 백로그(날짜 없음) 추가 시에는 일정 목록 생략.
