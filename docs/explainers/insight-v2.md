...
다만 확정이 느리다고 그 사이 침묵하는 건 아니다. 확정 전이어도 가능성이 보이면 emerging(검증중) tier로 계속 노출되고, e-value 진행바(`e=4.2/20`)가 확정까지 얼마나 왔는지 보여준다(§6-6).
...
> **읽는 법** (한 줄씩):
> - **발현일 n=8** — 이 시드가 지금까지 8번 켜짐 (발현 빈도는 이 절대 횟수)
> - **trigger 50% vs baseline 20%** — 시드 *켜진 날* 신호 발현율(50%) vs *안 켜진 날*(20%). 발현 빈도가 아니라 발현일에서의 발현율
> - **ratio 2.50x** — 켜진 날이 안 켜진 날보다 2.5배 자주 (50 ÷ 20)
> - **p · q** — Frequentist 우연 가능성 (작을수록 좋음)
> - **사후 50% [21%, 79%]** — Bayesian 확신도 + 95% 구간 (50%=우연 수준, 70\~80%↑=강함)
> - **e=4.2/20** — 증거 점수(§8 직관), 20 넘으면 "검증됨"
