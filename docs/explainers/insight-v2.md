...
- 어떤 5어휘(**시드·매트릭·매칭·가설·검증**)가 이 시스템을 떠받치는지
- 본인이 매일 기록만 하면 시스템이 어떻게 패턴을 발굴·채점하는지
- 사주는 이 시스템에서 무엇인지 (시드 출처 중 하나)
- 본 시스템이 무엇을 **할 수 있고** 무엇을 **하지 않는지**

**누가 읽으면 좋은가**

- 이 프로젝트에 관심 있어 처음 들여다보는 사람 (사주 모름 OK)
- 6개월 뒤의 본인 (구조를 잊지 않기 위해)
- 외부 리뷰어 / 협업자 (시스템 책임 경계를 이해해야 할 때)

> **읽기 전 약속**: 이 시스템은 사주를 증명하지 않는다. 사주는 "이미 정의된 결정론적 환경 변수 풀(pool)" 중 하나로 차용된다. 사주를 모르거나 안 믿어도 본 문서는 끝까지 읽을 수 있게 짜여 있다.
...
> **사람이 가설을 매번 직접 검토하지 않아도, 데이터가 쌓일수록 시스템이 스스로 신뢰도를 올린다.**

매일 일기·일정·수면·루틴·지출을 기록하면, 매일 09:00에 매칭 cron이 돌아가고, 매주 월요일 08:00에 가설 검증 cron이 돌아간다. 사람은 결과 잔소리 한 줄만 받는다.

사주는 이 흐름 안에서 "이미 결정되어 있는 환경 변수의 거대한 풀"로 차용된다. 본인 사주의 천간·지지·60갑자 같은 글자가 일운(그날의 운)에 들어올 때마다 시드가 발현된다. 사주의 옳고 그름을 묻는 게 아니라, 이미 정해진 글자들을 트리거로 쓸 뿐이다.

---

## 3. 전체 그림 한 장

매일 무엇이 어떤 순서로 일어나는지부터.

```mermaid
flowchart TD
  A["매일 기록<br/>일기·일정·수면·루틴·지출"] --> B["09:00 매칭 cron<br/>활성 시드 전부 평가"]
  B --> C["pattern_matches<br/>(hit / miss / inconclusive)"]
  C --> D["매주 월 08:00<br/>가설 검증 cron"]
  D --> E["pattern_stats<br/>Fisher's exact + BH-FDR<br/>+ Bayesian posterior"]
  E --> F["#insight 카드<br/>+ 09:00 잔소리 한 줄"]
```

같은 흐름을 5어휘로 다시 그리면 — 각 어휘가 어떤 어휘를 안에 담는지 nested 구조로:
...
- 시드는 위에 있고, 매트릭은 시드 안에 1\~N개 들어간다.
- 매칭은 매트릭이 하루 평가될 때마다 한 행씩 쌓이는 결과 기록.
- 가설은 시드와 매칭 결과 위에서 새로 만들어지는 별개의 자료(시드가 가설을 "낳는" 게 아니라, **시드 × outcome 조합**에서 통계가 가설을 발굴한다).
...
| 용어 | 한 줄 정의 |
|------|-----------|
| **트리거** (trigger) | 매일 09:00에 시드가 켜졌는지 결정하는 조건 |
...
> 사주 용어(천간·지지·십성·12운성·합/충/형 등)는 본 문서에 등장하지 않는다. 사주 시드가 어떻게 정의되는지 자세히 보려면 [사주 부록](insight-v2-saju.md)으로.
...
**핵심**: 두 출처 모두 **동일한 5어휘 파이프라인**을 통과한다. 사주만의 특수 처리 없음. 이게 본 시스템이 "사주 검증기"가 아닌 이유다.
...
> 위 표의 용어가 낯설다면 그게 정상이다. 본 문서는 사주 용어 사전이 아니라서 자세한 설명을 의도적으로 피했다. 깊이 들어가고 싶다면 [사주 부록](insight-v2-saju.md)으로.
...
본 시스템은 **두 가지 통계 도구를 동시에** 가설에 적용한다.
...
n=1(본인 1명) 환경에서 누적이 적은 단계는 Frequentist가 약하고 Bayesian이 안정적이다. 누적이 충분해지면 Frequentist의 효과량(rate_ratio)이 의미를 가지기 시작한다. 두 측이 같은 가설에 동의할 때 신뢰가 최대가 된다.
...
| **사주의 통계적 검증 불가능성** | 사주 자체가 "맞다 / 틀리다" 식으로 검증 가능한 대상이 아님 | 본 시스템은 사주를 "정의된 시드 풀"로만 다룸. 사주 자체 진위는 본 문서 범위 밖 |
...
2026-05-16      마스터 #434 시작 — 정체성 진화
                "본인 1명 패턴 발견 시스템 (사주는 시드 출처 중 하나)"
...
핵심 진화: **"사주 검증"** → **"본인 1명 패턴 발견 (사주는 시드 출처 중 하나)"**

같은 파이프라인이 사주 한정에서 라이프 통념 합류까지 일반화됐고, 그 과정에서 5어휘 모델이 직접 표현되는 `pattern_*` 스키마로 rename됐다. v2의 검증 헌장 4개는 그대로 남고, 그 위에 본인 1명 통계·5어휘 분리·target-type 일반화·승인 게이트·신뢰 비용 분리 5개 자체 헌장이 더해졌다.
...
이 한 문장이 본 시스템의 전부다. 사주가 시드 풀의 큰 부분을 차지하지만, 본 시스템은 사주를 증명하지 않는다. 사주든 라이프 통념이든 같은 5어휘 파이프라인을 통과하고, 같은 통계 두 측의 채점을 받는다.

본 문서를 다 읽었다면 다음 두 흐름으로 들어가 볼 수 있다.
