...
본문에 계속 나올 5가지 개념을 먼저 정의한다. 흐름 도식은 다음 섹션에서 본다.
...
| **매칭** (match) | 활성 시드마다 하루 한 행. 그 시드가 켜졌는지(trigger)와 매트릭별 채점 결과를 `metric_values`에 묶어 남긴다 | "2026-06-01 시드 #42 켜짐, hit" |
...
위에서 정의한 5가지 개념이 매일·매주 어떤 흐름으로 돌아가는지 도식으로 본다. **매일 사이클**과 **매주 사이클**이 따로 돌아간다.
...
같은 흐름을 **5가지 개념이 서로 어떻게 포개지는지** 관점으로 다시 본다.
...
**카운터는 매트릭 단위가 원칙**이다(ADR-0023). hit/miss/inconclusive와 Bayesian α/β를 `pattern_metrics`(매트릭 행)에 두고 시드 합계는 view로 모은다. 다만 지금은 시드에 매트릭이 보통 0\~1개라(거의 1:1), 시드 단위 hit/miss를 그 시드 매트릭에 그대로 반영한다. 한 시드에 매트릭이 2개 이상 붙으면(LLM 매트릭 추가) 매트릭별 `passed`로 나누는 분배 로직이 들어간다. 그 전까진 `metric_values`에 매트릭별 결과를 남겨두기만 한다.
...
같은 흐름이 사주 한정에서 라이프 통념 합류까지 일반화됐고, 그 과정에서 5가지 개념이 직접 표현되는 `pattern_*` 스키마로 rename됐다. v2의 검증 헌장 4개는 그대로 남고, 그 위에 본인 1명 통계·5가지 개념 분리·target-type 일반화·승인 게이트·신뢰 비용 분리 5개 자체 헌장이 더해졌다.
...
이 한 문장이 전부다. 사주가 시드 풀의 큰 부분을 차지하지만, 사주를 증명하지는 않는다. 사주든 라이프 통념이든 같은 5가지 개념 위에서 같은 흐름을 거치고, 같은 통계 두 측의 채점을 받는다.
