...
두 기준 모두 **출발점**이지 **결론**이 아니다. 시스템은 둘을 같은 5어휘 파이프라인 위에 올려놓고, 매칭이 쌓이면 통계로 가설을 발굴·채점한다. 시간이 지나면서 본인 데이터에 맞지 않는 시드는 자연스럽게 가라앉고, 본인 데이터에 자주 등장하는 시드만 활성으로 살아남는다 — **갈수록 본인에게 맞춰지는** 흐름.

> **사주의 옳고 그름을 묻는 게 아니다.** 사주는 매일 결정론적으로 들어오는 환경 변수의 풀로만 쓰인다. 사주가 통계로 의미 있는 시드를 더 많이 찾아주든 적게 찾아주든, 본 시스템은 동일하게 동작한다.

---

## 3. 전체 그림 한 장

매일 무엇이 어떤 순서로 일어나는지부터. **매일 사이클**과 **매주 사이클**이 따로 돌아간다.

```mermaid
flowchart TB
  REC["매일 기록<br/>(일기 · 일정 · 수면 · 루틴 · 지출)"]

  subgraph DAILY["매일 09:00 — 매칭 cron"]
    direction TB
  ...
  subgraph WEEKLY["매주 월요일 08:00 — 가설 검증 cron"]
    direction TB
...
- 09:00 cron 한 번에 **어제 채점 + 오늘 평가 + 잔소리** 세 일이 같이 일어난다.
- "오늘 시드 트리거 평가"의 결과(어떤 시드가 켜졌는지)는 그날엔 채점되지 않고, **다음 날 09:00 cron이 어제 데이터로 채점**한다. 시드가 켜진 날의 매트릭 평가에는 그날 종일 누적된 데이터가 필요하기 때문.
- 매일 사이클은 pattern_matches에 행을 쌓는다. 매주 사이클은 그 누적을 보고 가설을 발견·평가한다 — **둘은 다른 박스에서 다른 시간에 돈다**.

같은 흐름을 **5어휘가 서로 어떻게 포개지는지** 관점으로 다시 본다.
