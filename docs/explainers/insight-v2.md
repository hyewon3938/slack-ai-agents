...
이 시스템은 **나 한 사람의 데이터(n=1)**로 약한 신호를 잡는다. 표본이 나 한 명뿐이라 신호가 약하고, 약한 걸 잡다 보면 착각하기 쉽다. 그래서 통계 스택의 각 조각은 화려함이 아니라 **착각 하나씩을 막는 장치**다(정본 [ADR-0032](../adr/0032-metric-first-verification-statistics.md)). 전문 용어는 남기되, 무엇을 막는지 한 줄로 본다.
