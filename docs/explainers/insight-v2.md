...
자꾸 같은 패턴을 겪는 것 같다는 의심이 들 때, 그게 진짜인지 아니면 인지·확증 편향 탓인지 구분하기 어렵다.
...
매일 일기·일정·수면·루틴·지출을 기록하면, 매칭이 매일 돌아가고 가설 검증이 매주 돌아간다. 그렇게 찾아낸 결과는 잔소리로 받아보기만 하면 된다.
...
> **사주의 옳고 그름을 묻는 게 아니다.** 사주는 매일 결정론적으로 들어오는 환경 변수의 풀로만 쓰인다. 사주에서 통계적으로 의미 있는 시드가 많이 나오든 적게 나오든, 시스템은 똑같이 동작한다.
