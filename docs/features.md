...
- 운 레벨 차원(마스터 #434 Phase 2.5): 시드별 `pillar_level`(원국/대운/세운/월운/일운) 차원 도입. 누적 카운트 시드(화 오행·편재 N=1..5)·`cumulative_pillar_count` trigger·월 09:15 분포 분석 cron은 #508(ADR-0046)에서 강도 밴드(ADR-0036)로 위임·은퇴 — 고정 임계 누적이 baseline 포화 시 죽는 문제를 상대 분위수 밴드가 흡수. 트리거 코드만 휴면 보존
