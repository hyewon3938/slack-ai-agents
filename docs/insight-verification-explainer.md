...
> 사람이 매번 가설을 직접 검토하지 않아도, 데이터가 쌓일수록 가설은 스스로 `active → confirmed → rejected` 상태를 옮겨간다.
...
이 프로젝트는 두 가지 출발점에서 시작했다. 둘 다 *어떤 주장이 본인 데이터에서 실제로 작동하는가* 를 자동으로 확인하고 싶다는 같은 문제로 수렴한다.

### 출발점 1. LLM 잔소리는 검증되지 않는다

LLM에게 일기 3개월치를 보여주고 어떤 패턴이 있는지 물으면 그럴듯한 답이 나온다. 문제는:

1. **그 답이 정말 맞는지 확인할 방법이 없다.** LLM은 같은 질문에 매번 다른 답을 줄 수 있고, 본인은 그 답을 검증할 시간이 없다.
2. **사람은 매번 확인 못 한다.** 매일 일기 쓸 때마다 이번 주 가설을 다시 점검하는 일을 사람이 직접 할 수 없다.
3. **직관적으로 그럴듯한 가설은 실제로 틀린 경우가 많다.** 인지편향(확증편향, 회상편향) 때문에 본인이 느끼는 패턴은 실제 데이터와 어긋난다.

### 출발점 2. 효과 있다고 알려졌지만 검증된 적 없는 도메인이 있다

라이프스타일 영역에는 효과 있다고 널리 알려진 통념이 많다 — 수면시간 지키면 번아웃 안 온다, 운동하면 기분이 올라간다, 일찍 자면 다음날 컨디션이 좋다. 명리학 같은 외부 결정론(특정 일주에는 특정 경향이 나타난다)도 비슷한 위치에 있다. 공통점은 **개인 데이터로 본격적으로 검증된 적이 거의 없다**는 것.

이 도메인들을 본인 데이터로 끌어들여 같은 검증 시스템으로 채점하면, 어떤 통념이 본인에게 진짜 작동하는지 정량적으로 알 수 있다. 명리학을 신비주의로 다루는 게 아니라, **검증 대상의 한 사례**로 끌어들인 것.

### 해결 방향

두 출발점을 같은 원칙으로 푼다.

> **LLM과 외부 통념은 가설 제공자로 적극 활용하되, 그 출력은 통계·DB·도구 권한으로 자동 검증한다.**

- LLM과 외부 도메인(명리학·라이프스타일 통념)이 가설을 *제공*한다 (창의성·기존 지식 활용)
- DB와 통계 검정이 그 가설을 *채점*한다 (결정론적 검증)
- 사람은 통과한 가설만 잔소리로 받아본다 (인지 부하 최소화)

## 3. 전체 그림 한 장

```mermaid
flowchart LR
    A[일기/수면/지출/일정/루틴 입력] --> B[(PostgreSQL)]
    B --> C[일일 매칭 cron]
    C --> D[주간 가설 채점 cron]
    D --> E{통계 검정 통과?}
...
큰 흐름은 이렇다:
- **입력** — 일기·수면·지출·일정·루틴이 Slack/웹으로 들어와 DB에 쌓임
...
| **trigger** | 시드가 오늘 발현했는지를 SQL로 판정하는 결정론적 조건 | 60갑자 일주가 특정 천간/지지인 날 = 발현 |
...
| **가설 (hypothesis)** | trigger 발현일에 outcome이 평소보다 자주 일어난다는 명제 | *비견 일주에는 `social_activity`가 평소보다 자주 일어난다* |
...
| **view (saju_influence_summary)** | Phase 3 시드 카탈로그 + Phase 4 confirmed 가설을 단일 SQL view로 합친 것 | 다른 도메인은 현재 어떤 패턴이 active인지만 보면 됨 |
