...
## 2026-07-31: 결제수단 출금 시점 모델 도입 (#615, PR #616)

예산 층과 자금 층이 서로 다른 시간축을 쓰던 구조를 **기준일**이라는 단일 좌표로 정합화.

- 결제수단 정의를 한 파일로 모으고 출금 시점(즉시/후불)을 1급 속성으로 승격 — 카드 대금기간 상수도 여기서 파생
- `assets.balance_as_of` 도입 — "이 잔액은 며칠까지의 입출금을 반영한 값인가". 예산 기준선을 저장값에서 계산으로 분리해, 잔액을 언제 입력해도 같은 기준선이 나온다
- 주기 정산은 기준일까지 이미 반영된 몫을 빼고 미반영분만 적용한 뒤 기준일을 전진 (마이그레이션 109)
- 고정비별 결제수단 설정 + 변동 고정비의 그 달 금액 수정 지원

설계 판단 → [ADR-0062](adr/0062-payment-method-withdrawal-timing.md). 서사 → [design-notebook/budget-model-simplification.md](design-notebook/budget-model-simplification.md).

---

## 2026-07-10: 수면 인사이트 강화 마스터 close (#588)

수면 도메인을 단일 점수에서 다축 평가 + 세그먼트 합성 + 양방향 입력으로 확장한 마스터가 Phase 1\~3 머지+배포로 완결.

- **Phase 1 (점수 다각화 + 세그먼트 집계, PR #592)**: 수면을 4축(시간·규칙성·연속성·타이밍) + 종합 점수로 재설계. 분할 수면(같은 날 밤잠 여러 건)을 세그먼트로 합성해 수면 효율·WASO(수면 중 각성) 파생. 대시보드 수면 탭 개편.
- **Phase 2 (슬랙 입력 정밀화 + 봇 세그먼트 정합, PR #594)**: 슬랙 수면 입력을 세그먼트 스키마에 정합시키고 신호 축 확장(마이그레이션 107).
- **Phase 3 (대시보드 입력 폼, PR #596)**: 밤잠 세그먼트·낮잠·중간기상·특이사항 태그·메모를 웹에서 직접 입력하는 CRUD 폼 — 슬랙과 동등한 입력 경로.

설계 판단 → [ADR-0059](adr/0059-sleep-score-architecture.md). 마스터 서사 → [design-notebook/sleep-insights.md](design-notebook/sleep-insights.md).

---

## 2026-07-10: 일정 삭제 사유 수집 (#590, PR #591)

일정 삭제 시 사유를 수집해 tombstone으로 축적하고, 변심 취소를 패턴 신호로 승격.

- 웹은 사유 선택 모달(고정 5종 + 기타 텍스트), 슬랙은 대화로 사유 수집
- 삭제 확인 UX 단일화 — 중복 알럿 제거
- tombstone enrichment로 사유 누적 + `audit_cancel_changed_mind` 시드 신호로 패턴 검증축에 연결 (마이그레이션 106)

설계 판단 → [ADR-0060](adr/0060-schedule-delete-reason-capture.md).

---

## 2026-07-05: 사주 심층 리포트 통합 (#581\~583, PR #584\~586)

사주 해석 개인화 코어를 콜 장부 + 태그 확장 + 구세대 프롬프트 정리로 통합.

- **콜 장부(`fortune_calls`, #582/PR #584)**: 일운 예측을 사전 등록 → 주간 엔진이 밤마다 방향 적중을 채점 → 월요일 통합 카드에 지난주 콜 판정 노출. 예측-검증 루프를 사주 일운에 적용.
- **일기 메타 태그 확장(#581/PR #585)**: 일기 LLM enum 어휘를 22 → 26종으로 확장(마이그레이션 102).
- **구세대 프롬프트 참조 은퇴(#583/PR #586)**: 갱신이 멈춘 `saju_patterns` 봇 프롬프트 참조 제거 — 역할은 프로필 요약 + `pattern_links` 검증축이 대체.

설계 판단 → [ADR-0057](adr/0057-fortune-calls-ledger.md). 마스터 서사 → [design-notebook/fortune-rework.md](design-notebook/fortune-rework.md).

---

## 2026-07-04: 업타임 모니터링 이중화 (#577, PR #578)

GitHub Actions 폴링(2차 관측)에 외부 관측 기반 dead-man's-switch(1차)를 더해 관측 경로를 이중화.

- 봇이 5분 간격 heartbeat를 외부 관측 서비스로 전송 → 신호 끊김 감지 시 Slack 알림
- 봇·웹을 각각 독립 check로 분리
- 중앙 스케줄러 스로틀링으로 폴링 간격이 벌어져도 다운 감지 누락을 방지

설계 판단 → [ADR-0055](adr/0055-uptime-deadman-heartbeat.md). 운영 가이드 → [docs/ops/health-monitoring.md](ops/health-monitoring.md).

---

## 2026-07-02\~03: 전체 시스템 감사 + 도메인 견고화 (#549\~#559, PR #560\~#570)

예산·인사이트 두 도메인을 대상으로 한 계획적 전체 감사에서 도출한 11건을 일괄 정리·배포.

- **예산 견고화**: 결제주기 정산 누락 보정 + 고정비 기록 보장(#551), 지출 전망 카드 계획·실지출 이중 전망(#552), 목표 기간 만료 임박 알림(#554), 일별 예산 로그 백필 산식 단일화(#550), 할부 지출 예산 플래그 정합성 정리(#549)
- **인사이트 정밀화**: 역방향 링크 종결 규칙 + 반대 방향 가설 재제안(#555), 교란 탐지 데이터-존재 윈도우 적용(#556), 기각 이력 신호 재생성 가드(#557)
- **정리·검증**: 레거시 코드·미사용 테이블 정리(#553), 절기 전환 감지 경로 테스트 보강(#558), 도메인 문서·크론 표 현행화(#559)

마이그레이션 095·097\~099 포함, 전 항목 배포 후 프로덕션 검증. 반응형 대응이 아니라 정기 감사를 엔지니어링 루틴으로 운영한 사례.

---

## 2026-06-16: 주간 인사이트 단일 카드 통합 (#542, PR #544)

봇이 발송하던 별도 검증 카드를 은퇴시키고, 주간 회고·라이프 메트릭·패턴 학습을 routine 통합 카드 한 장으로 단일화.

- 봇 주간 검증 엔진은 DB만 갱신(카드 미발송) + 발굴 후보 카드만 발송
- routine 통합 카드(월요일 08:04)가 회고 + 메트릭 + 패턴 학습(검증중/검증됨/기각)을 단독 노출
- posterior 등 내부 수치 노출 제거, 라벨 단축

설계 판단 → [ADR-0052](adr/0052-weekly-insight-single-card-merge.md).

---

## 2026-06-13: 세운·대운 확장 + 검증 교정 마스터 close (#523)

일운 중심이던 사주 검증을 월운·세운·대운 기간 레이어로 확장하고, 검증가능성 사다리에 맞춰 측정·해석을 분리한 마스터가 Phase 0\~3 머지+배포로 완결.

- **Phase 0\~1 (측정 교정 + 집계)**: 순변위 측정 교정 + 기간별 반응 프로파일 집계(마이그레이션 088)
- **Phase 2 (해석 + fast path + 절기)**: `period_interpretations` — 절기/입춘/대운 전환 시 기간 해석 카드 생성 + 결정론 fast path 조회(마이그레이션 089·090)
- **Phase 3 (예측 장부, PR #532)**: `period_forecasts` — 월운·세운 예측 사전등록 → 사후 채점(방향 적중 + 실측 delta). baseline 동결·절기 시간게이트 무인 자기검증. 대운은 검증불가로 비편입(마이그레이션 091)
- **검증가능성 사다리 완결**: 일운(실증) → 월운(축적 실증) → 세운(장부 약실증) → 대운(비실증) 4층

설계 판단 → [ADR-0049](adr/0049-response-profile-aggregation.md), [ADR-0050](adr/0050-verifiability-ladder-and-forecast-ledger.md). 마스터 서사 → [design-notebook/period-extension.md](design-notebook/period-extension.md).

---

## 2026-06-08: 발굴 엔진 측정 타당성 + 카드 가독성 마스터 close (#504)

#477 발굴 파이프라인의 측정 타당성을 교정하고 카드 가독성·재추천 흐름을 정비한 마스터가 Phase 1\~3 머지+배포로 완결.

- **Phase 1 (측정 타당성, ADR-0044)**: 데이터-존재 윈도우 + 효과크기 랭킹 + 중복 신호 정규화 — 결측을 비발현으로 오판하던 문제 제거
- **Phase 2 (카드 가독성, ADR-0045)**: 시드·신호 변수명 → 자연어 라벨 레이어(`src/shared/insight-labels.ts`, 런타임 번역·DB 미변경)
- **Phase 3 (후보 재추천, PR #513, ADR-0047)**: 주간 묶음 전부 패스 시 다음 best 후보를 데일리 재추천하는 전용 슬롯
- 신호·시드 정밀화(날짜 방향 분리·강도밴드 위임·포화 양방향 가드, ADR-0046)도 후속으로 편입

마스터 서사 → [design-notebook/discovery-refinement.md](design-notebook/discovery-refinement.md).

---

## 2026-06-06: 매트릭 중심 패턴 검증 마스터 close (#477)

#434의 5어휘(시드→매트릭→매칭→가설→검증)를 **정량 매트릭 1차 검증축**으로 재설계한 마스터가 P1\~P7 머지+배포로 완결. 일기 주관 태그에 의존하던 검증 축을 객관 정량 신호(수면·루틴·일정·지출)로 끌어올리고, n=1 단일 케이스에서 통계적으로 정직한 패턴 검증 파이프라인을 세웠다.

- **P1\~P2 (신호 전역화 + 주간 검증 엔진)**: 시드종속 매트릭 → 전역 신호 + (시드×신호) 가설 모델. off-day 대조(발현일 vs 비발현일) 2×2 → Fisher + BH-FDR + Beta-Binomial.
- **P3 (통계 보강)**: e-value 순차 확정 게이트(anytime-valid) + verified/emerging/recent 3-tier 노출 + block permutation 자기상관 보정.
- **P4a\~P4b (사주 feature substrate)**: 실효강도·상대 분위수 밴드·합화 변환·관계를 전부 결정론 binary feature로 환원해 단일 off-day 엔진에 태움(새 통계 코어 없이 차원 확장).
- **P5a\~P5b (발굴 + LLM 신호 제안)**: 여집합 off-day 스캔으로 미지 패턴 발굴 + LLM 자율 SQL 신호 제안(정적 검증 + 격리 실행 2단 방어). 둘 다 사람 승인 게이트 → 통계가 최종 심판.
- **P6\~P7 (교란 통제)**: 공동발현 제3변수 어부지리를 marginal 플래그(P6) → Mantel-Haenszel 층화 조정(P7)으로 노출에서 제거. 조정은 노출 레이어에만 반영해 e-value 결정론 보존.
- **설계 자산**: ADR-0032\~0042(11개), design-notebook `metric-first-verification.md`(헌장 4개 + P1\~P7 서사 + 마스터 회고). 데이터 게이트 자동 활성 원칙(헌장 ④)으로 교란 분리·발굴·LLM 제안을 dormant 빌드 — 데이터가 차면 스스로 켜진다.

첫 주간 검증 엔진(P2\~P7 결합)은 2026-06-08 06:00 첫 실행.
