...
### 회고 (2026-07-04, PR #579 머지·배포 후)

- **에이전트의 사실 주장도 원천과 대조한다.** 구현 에이전트가 "expenses에 type 컬럼 없음"이라며 구 신호의 `type='expense'` 필터를 승계하지 않았고, 그 오판이 ADR·도메인 문서 본문까지 전파돼 있었다. 030 원본 DDL과 prod 스키마 실측(`information_schema`)이 반박 근거 — 리뷰에서 필터 복원. 2차 회고("계획서의 전제도 리뷰 대상")와 대칭으로, 이번엔 구현자의 전제가 리뷰 대상이었다. 필터 누락은 에러가 안 나는 클래스라 실행 스모크로도 못 잡는다 — 의미 보존은 사람(리뷰) 몫.
- **실측 쿼리의 실패는 노이즈가 아니라 신호다.** 항목 1의 done 비율을 재려던 쿼리가 `column "category" does not exist`로 터진 것이 좀비 신호 발견의 전부였다 — 분석 에이전트도 로컬 SQL 원문만 봐서 몰랐다. "점검하려다 깨진 자리"가 이슈의 원래 스코프(의미론 6건)를 넘는 7번째 문제(스키마 드리프트 클래스)를 드러냈고, 결과적으로 이번 PR의 헤드라인이 됐다.
- **마이그레이션이 자기 산출물을 실행해 증명한다.** 100의 트리거 자기검증(임시 행 왕복)이 101에선 신설 SQL 실행 스모크로 진화 — "정의를 넣었다"가 아니라 "정의가 현행 스키마에서 실제로 돈다"를 적용 시점에 봉인. catalog(DB)에 영속되는 SQL의 타입 체크 사각지대를 메우는 자리는 여기뿐이다.
