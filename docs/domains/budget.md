...
- **런웨이 계산**: 자산 / 월 평균 지출로 재정 지속 가능 기간 산출
...
실제 금액, 자산 현황, 런웨이 수치 등 민감 정보는 `docs/budget-internal.md` (gitignored) 참조.
이 문서에는 기능 구조와 설계만 기록하며, 개인 재정 데이터는 포함하지 않는다.
