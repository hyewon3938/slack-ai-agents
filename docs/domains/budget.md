...
> **진행 중**: 모델 단순화 (#539) — 아래 골격 참조.

## 모델 단순화 (#539) — 진행 중

> TODO(`/build`): 구현 후 본문 채우고, **이 리팩토링이 무효화하는 기존 서술을 같은 PR에서 갱신**한다. ADR [0051](../adr/0051-budget-model-simplification-runway-locked.md) 기준.
>
> **/build가 갱신해야 할 기존 섹션 (현행 서술이 새 모델과 어긋남):**
> - `핵심 개념 > 가용자금` — "할부 2+회차 INSERT 즉시 차감" 서술 제거, "자금 = 현금 유동자금 단일 필드"로 교체
> - `핵심 개념 > 일일 예산` — 입력 변화(할부 1회차 flex 제외) 반영, 산식(`todayBudget`/`todayRecommended`)은 불변 명시
> - `제외 플래그 모음` — `distribute_to_runway` 행 제거(데드 컬럼화), 할부 `exclude_from_budget` 무관 명시
> - `DB 스키마 > assets` — 자금 단일화 반영, `expenses.distribute_to_runway` 데드 컬럼 주석
> - `기능 구현 상태` 표 14·15(할부 자산 차감 토글/그룹 동기화) — 제거/은퇴 처리
> - `데이터 흐름 > 할부 자산 차감 범위 토글` / `할부 exclude 토글` 블록 — 삭제
> - `주요 계산 규칙` — 묶인 돈 = 목표기간 창 일괄 reservation 규칙 추가, depletion(정산) 분리 명시
> - `향후 개선 과제` — 목표기간 만료 경고 항목 추가
>
> **새로 기술할 내용 골격:**
> - 자금 단일화 (입력 의미 = 현금 유동자금, 비상금 분리)
> - 묶인 돈 = `[현재 결제월, target_date]` 창 안 [할부 회차 + 고정비 + 예정] 일괄, 창 밖 무시
> - reservation(라이브 계산) vs depletion(정산 시 전체 결제분 차감) 분리
> - 할부 1회차 = 묶인 돈(현재월 락), flex 제외
> - 제거된 것: 건별 토글, target_date 변경 자산 보정, 등록 시점 자산 차감
> - 마이그레이션 번호 / 변경 파일 요약
