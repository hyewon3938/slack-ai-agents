...
> TODO(`/build`): 구현 후 본문 채우기.
> - `routine_templates.tracking_mode`(`scheduled` | `free`)와 `routine_records.entry_type` 스탬프의 의미·불변성
> - 자동 생성 차단 지점 (봇 크론 / 웹 오늘 기록 보장 / 백필)
> - 측정 격리 — `entry_type = 'scheduled'` 조건을 적용한 소비처 목록 (DB 신호 정의 SQL·봇 집계·웹 통계·LLM 프롬프트)
> - 주기형 기록 하루 1건 보장 (조건부 유니크 인덱스), 자율 기록 하루 여러 건 허용
> - 모드 전환 시 정합 처리 + 주기형 전용 항목(빈도·시작일·비활성 기간)의 취급
> - 웹 기록 경로 (체크리스트 하단 자율 루틴 섹션 → 기록 모달)
> - 상단 `## DB 스키마` 블록에 신규 컬럼 2개 반영 + `## API 엔드포인트` 표 갱신
