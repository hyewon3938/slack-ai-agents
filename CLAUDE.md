...
- main에 머지하면 GitHub Actions가 운영에 자동으로 배포하므로 `/deploy`를 쓰지 않는다. 머지 뒤 `LOCAL-TRACK.md`에 확인 묶음을 만들 때 머리 줄에 배포가 끝난 시각을 함께 적는다.
...
- 작업 중에 세션 범위 밖의 일이 생기면 같은 브랜치에 섞지 말고, 이슈를 따로 만들어 다른 세션에서 진행하길 제안할 것
